...
interface AtIterator {
    index: number;
    middle: number;
    call<Item>(this: AtIterator, context: LinkedListType<Item>, element: Item, index: number, iteration: IterationType<Item>): void;
}
...
export function at<Item>(this: LinkedListType<Item>, index: number) {
    ...
    const iterator = new (AtIterator as unknown as AtIteratorConstructor)(index, this.length);
