...
LinkedList.prototype.remove = function<Item>(this: LinkedListType<Item>, item: LinkedListItem<Item>) {
    if (item === this.last) {
        this.last = item.prev;
    }
    if (item === this.first) {
...
LinkedList.prototype.each = function<Item, Context>(
    ...
    iterator: (this: Context, item: Item, index: number, iteration: IterationType<Item>) => void,
    context: Context,
) {
